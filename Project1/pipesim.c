...
int timetaken = 0;                //Stores time taken to complete simulation
int PID_state[MAX_PROCESSES + 1]; //Stores the state of each process
int current_system_call[MAX_PROCESSES + 1]; //Stores the current index number of the system call in process_tracker for each PID

/*
* For each PID stores a list of the system calls made by that PID and the parameters of the call
* the simulation loops through each active process id and executes the current system call
* from that process.
*/
struct ProcessTracker
{
    ...
    int arg0;
    int arg1;
} process_tracker[MAX_PROCESSES + 1][MAX_PIPE_DESCRIPTORS_PER_PROCESS + 1];

/*
* PipesIn and PipesOut keep track of the active pipes for each 
* PID and the data they contain.
*/
struct PipesOut
{
    bool active;
...
//-------------------------- PROCESS QUEUE -------------------------
int process_queue[MAX_PROCESSES + 1]; //Stores queue of processes waiting to run
int front = 0;
int rear = -1;
int itemCount = 0;
...
void insert_process(int data)
{
    if (itemCount != MAX_PROCESSES + 1)
    {
...
/*
* Adds a system call to specified process queue.
*/
void add_process(int PID, int call, int arg0, int arg1)
{
    int process_number = current_system_call[PID];
    //printf("adding process PID:%i, call:%i process:%i\n", PID, call, process_number);
    current_system_call[PID]++;
    process_tracker[PID][process_number].call = call;
    process_tracker[PID][process_number].arg0 = arg0;
    process_tracker[PID][process_number].arg1 = arg1;
}

/*
* Changes the state of a process to 'state', increases 
* timetaken by value specified in the constants above.
*/
void change_state(int PID, int state)
{
    printf("@%i P%i: %s -> %s\n", timetaken, PID, state_descriptions[PID_state[PID]], state_descriptions[state]);
    PID_state[PID] = state;
    timetaken += USECS_TO_CHANGE_PROCESS_STATE;
}

//  ------------------------ SYSTEM CALL SIMULATIONS ----------------------------
/*
* Simulate the exit() system call, terminates process and closes pipes
*/
...
/*
* Simulate the compute() system call, will take up CPU for 'time'
* or until the time quantum has passed. If calculation time is still
* left then call repeates until time left is 0.
*/
void sim_compute(int PID, int time)
{
    ...
    else
    {
        ...
        process_tracker[PID][current_system_call[PID]].arg0 = time;
    }
...
/*
* Simulate the fork() system call, creates a new child process with 
* identifier 'childPID'. Connects the reading end of any active 
* pipes the parent has to the new child process.
*/
void sim_fork(int PID, int childPID)
{
    ...
    change_state(PID, READY);
    insert_process(childPID);
    insert_process(PID);
    current_system_call[PID]++;
}

/*
* Simulates the wait() system call, a process that calls this ceases
* operation until the specified child process 'childPID' has the
* 'EXITED' status.
*/
void sim_wait(int PID, int childPID)
{
    if (PID_state[PID] != WAITING)
    {
        printf("P%i:WAITING for childPID: %i\n", PID, childPID);
        change_state(PID, WAITING);
    }
    if (PID_state[childPID] == EXITED)
    {
...
/*
* Simulates the sleep() system call, process is marked as SLEEPING until 
* at least the allocated time has passed. Afterwhich the process is 
* marked as READY again.
*/
void sim_sleep(int PID, int time)
{
    if (PID_state[PID] != SLEEPING)
    {
        ...
        process_tracker[PID][current_system_call[PID]].arg0 += timetaken;
    }

    int sleep_time = process_tracker[PID][current_system_call[PID]].arg0;
    if (sleep_time < sleep_tracker)
        sleep_tracker = sleep_time;

    if (sleep_time - timetaken <= 0)
    {
...
/*
* Simulates the pipe() system call, process opens a unidirectional 
* pipe with specifed id 'desc'. The process is set as the writing end
* of the pipe. The pipe's reading end is unallocated unil fork() is called.
*/
...
/*
* Simulates the writepipe() system call, writes a specified amount of
* bytes to pipe 'desc'. If the pipe is full then the process is locked
* until more space in pipe becomes availiable.
*/
void sim_write(int PID, int desc, int bytes)
{
    ...
    if (pipe_out_tracker[PID][desc].nbytes > pipesize_bytes)
    {
        ...
        process_tracker[PID][current_system_call[PID]].arg1 -= pipesize_bytes - nbytes;
    ...
    if (PID_state[read_PID] == READBLOCKED && process_tracker[read_PID][current_system_call[read_PID]].arg0 == desc)
    {
        printf("P%i: can read its pipedesc=%i\n", read_PID, desc);
        change_state(read_PID, READY);
        insert_process(read_PID);
    }
}

/*
* Simulates the readpipe() system call, reads a specified amount of
* bytes from pipe 'desc'. If not enough bytes are availiable in the pipe
* then the process is locked until more bytes are written to the pipe.
*/
void sim_read(int PID, int desc, int bytes)
{
    ...
    { //not enough bytes to read
        process_tracker[PID][current_system_call[PID]].arg1 -= nbytes;
    ...
    if (PID_state[write_PID] == WRITEBLOCKED && process_tracker[write_PID][current_system_call[write_PID]].arg0 == desc)
    {
        printf("P%i: can write to pipedesc=%i\n", write_PID, desc);
        change_state(write_PID, READY);
        insert_process(write_PID);
    }
}

/*
* Runs the simulation, runs through queue of processes and executes the current
* system call being made by that process. Once process queue is empty eg. all
* processes have exited, then the simulation halts.
*/
void run_simulation() 
{
    //Reset everything and add the first PID to the queue
    for (int i = 0; i < MAX_PROCESSES; i++) 
    {
        current_system_call[i] = 0;
    }
    ...
    int old_timetaken = 0;
    while (!is_process_queue_empty()) //Cycles through process queue until all processed are terminated
    {
        int current_PID = remove_process(); //remove top process
        if (PID_state[current_PID] == READY)
        {
            change_state(current_PID, RUNNING);
        }
        else if (PID_state[current_PID] == SLEEPING)
        {
        ...
        else if (PID_state[current_PID] == READBLOCKED || PID_state[current_PID] == WRITEBLOCKED)
        {
            insert_process(current_PID);
            continue;
        }
        else if (PID_state[current_PID] == EXITED)
        {
            continue;
        }

        // Grab the current system call and arguments from current PID
        int call = process_tracker[current_PID][current_system_call[current_PID]].call;
        int arg0 = process_tracker[current_PID][current_system_call[current_PID]].arg0;
        int arg1 = process_tracker[current_PID][current_system_call[current_PID]].arg1;

        switch (call) //simulate current system call
        {
            case CALL_EXIT:
...
#define CHAR_COMMENT '#'
    ...
    //  add the system calls to process_tracker
...
int main(int argc, char *argv[])
{
    ...
    printf("file is %s\n", eventfile);
    //parse eventfile and fill processes
    parse_eventfile(argv[0], eventfile);
    //run the simulation
    run_simulation();

    //print timetaken for simulation to finish
    printf("timetaken %i\n", timetaken);
    return 0;
}