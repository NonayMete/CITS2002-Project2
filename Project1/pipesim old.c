...
{
    printf("@%i P%i: %i -> %i\n", timetaken, PID, PID_tracker[PID], state);
...
void run_simulation()
{
    ...
    while (!isEmpty())
    {
        ...
        if (finished)
            break;
        if (PID_tracker[current_PID] == NEW)
        {
            current_PID = 1;
            //printf("new\n");
            continue;
        }
        else if (PID_tracker[current_PID] == READY)
        {
            //printf("ready\n");
        ...
        else if (PID_tracker[current_PID] == EXITED)
        {
            ...
            while (1)
            {
                if (PID_tracker[i] == NEW)
                    break;
                if (PID_tracker[i] != EXITED)
                {
                    finished = false;
                    break;
                }
